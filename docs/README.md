# Documentation Index

## Product

- `product/product-requirements.md`
- `product/metrics.md`
- `product/glossary.md`
- `product-brief.md`

## Architecture

- `architecture/system-architecture.md`
- `architecture/data-flow.md`
- `architecture/data-model.md`
- `adr/`

## UX/UI

- `ux/screen-map.md`
- `ux/user-flows.md`
- `ux/design-system.md`

## Frontend

- `frontend/mobile-architecture.md`
- `frontend/mobile-best-practices.md`

## Backend

- `backend/backend-architecture.md`
- `backend/api-design.md`
- `backend/backend-best-practices.md`

## AI

- `ai/assistant-behaviour.md`
- `ai/rag-strategy.md`
- `ai/evaluation-plan.md`

## Tax

- `tax/tax-rules-strategy.md`
- `tax/tax-rule-update-playbook.md`

## Operations, QA and Security

- `security/security-model.md`
- `qa/test-strategy.md`
- `qa/multilingual-copy-checklist.md`
- `operations/release-checklist.md`
- `compliance-checklist.md`
- `roadmap/roadmap.md`
- `roadmap/epics.md`
- `roadmap/current-plan.md`
- `roadmap/tasks.md`
